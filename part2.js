...
function ribbonCalculator(boxes) {
  ...
  for (var i = 0; i < boxes.length; i++) {
    var boxDimensions = boxes[i].split('x').map(function(item) {
    return parseInt(item, 10);
    });

    boxDimensions.sort(function(a, b){return a-b});

    var perim = 2*(boxDimensions[0] + boxDimensions[1]);
    var bow = boxDimensions[0]*boxDimensions[1]*boxDimensions[2];
    ribbonNeeded += (perim + bow);
  }
    return ribbonNeeded;
};
